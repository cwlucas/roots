...
module.exports = function(file, ref, strip){
  f = {};
  ...
  f.extension = path.extname(file).slice(1);
  f.target_extension = path.extname(f.export_path).slice(1);
  ...
  function write(write_content){
    ...
    // check for `strip`, if present strip the last extension
    if (strip) {
      var ep = f.export_path;
      var new_path = ep.replace(path.extname(ep), '');
      fs.renameSync(ep, new_path);
      f.export_path = new_path;
    }
  }

  function compress(write_content){
    require('./compressor')(write_content, f.target_extension)
  }
